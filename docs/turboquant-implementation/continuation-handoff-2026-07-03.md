...
### 2026-07-10/12: AC CAMPAIGN COMPLETE — consolidated verdicts

Full data: `artifacts/campaign-20260710-verdicts.md` (+ per-phase artifact dirs). All measurements real-model, engagement-verified, guarded, per the ruling. One incident: the unguarded acceptance harness + a 32K fp16-KV arm hard-crashed the 16GB host → **memory guard ported into TurboQuantAcceptanceHarness + N7 protocol 16K default cap (e7c7335)**; benchmark infra now has both permanent protections (sleep-voiding + memory guard).

| # | Question | Verdict |
|---|---|---|
| M1 | Is the affine SDPA kernel slow at the REAL Qwen3.5 shape? | **YES — 0.304× fp16/call @16K (BW ratio 0.128), 0.448× @32K.** H1 confirmed+sized; the old "kernel exonerated" was synthetic-shape-only. |
| M2 | Is the blocks=256 split-k default the cause? | **NO — ratio identical (0.718) at blocks 64/256** (knob verified live on the isolated kernel). Deficit is KERNEL-INTERNAL (H1b register regime @D=256 / H1c pass-2 underfill). **⇒ F2 kernel restructure (BD=8 @D=256, mlx core) is the flagship build; F4 host diet (~12%) the quick win.** |
| P3 | Does the native built-in coop branch pay on a real model? | **YES — +4.2% @16K → +12.3% @32K** (kinds [3]↔[4] engagement-clean, 10/10 arms; default-ON ≥32K already). **Floor stays 32768.** Coop workstream CLOSED. Caveat: turbo4v2 fails the quality tail bar (p95 9.2–10.9 vs 2.0; top1=1.0) on every arm → PolarQJL = memory-reach only, unpromotable pending the tail question. |
