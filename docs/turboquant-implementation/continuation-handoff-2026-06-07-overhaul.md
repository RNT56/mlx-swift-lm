...
| PolarQJL metadata diet | realize paper 4–7× compression | **N4 codec INTEGRATED + VALIDATED** (mlx-swift `dc6b9bd`+`4a83f63`): `TurboQuantReferenceFormat.gaussianLloydMax` data-free Gaussian quantizer landed in encode/decode dispatch; round-trip 5-bit 2.91× @cos 0.9987 (≈codec 2.0×), 3-bit 4.57× @cos 0.982. 4–7× lever = PAYLOAD quantizer not metadata. Remaining = Metal `.gaussianLloydMax` decode kernel (fast path) |
...
- **Admission rule (data-backed by N1):** floor default 8192 (prefetch erases the ≥8K regression;
  ...
  tuning constant.
- **Pines N2 — APPLIED + VALIDATED in the working tree (2026-06-08), uncommitted pending the
  A-series device cycle.** The prior-agent WIP was committed first (pines `5341015`, green: 227
  PinesCore tests). Then, in the pines working tree (branch `tq/real-device-evidence-acceptance`,
  changes **left uncommitted** by user choice — hold the device run):
  - **Pin bumped** to mlx-swift `6bfa04e2924152c52c56eac5c3420a7cc7e8d720` + mlx-swift-lm
    `295e66bef0b3d85be70290b5c1adea83d694660c` across the 5 config/code sites (`project.yml`,
    `project.pbxproj`, `Package.resolved`, `docs/TURBOQUANT.md`, the bridge `turboQuantCompatibilityPairID`).
    NOTE the pair is `(6bfa04e, 295e66b)` — mlx-swift-lm@295e66b's `Package.swift` pins mlx-swift `6bfa04e`
    (N4's `4a83f63` is NOT consumed; no Metal kernel wired). `compatibility-pair.json` is deliberately
    left at the OLD pair (it is harness-generated evidence; hand-editing it = fabrication, guardrail-blocked).
  - **N2 wired into `MLXRuntimeBridge`** (both `#if PINES_TQ_WAVE6_API`/`#else` GenerateParameters):
    `selfSpeculationMode: promptTokenCount >= selfSpeculationMinPromptTokens ? .promptLookup : .off`,
    `selfSpeculationPrefetch: true`, `selfSpeculationMinPromptTokens: 12288` (new static). Admission-gated,
    bit-exact greedy, safe fallback for temp>0/non-trimmable hybrid.
  - **All pin-jump API drift fixed** (4 exhaustive switches: `TurboQuantAttentionPath`→engine adds
    `.nativeMLXCompressed`/`.affine*Native`/`.affineK8VxResidual`→`.nativeMLX` + `.sparseValueTwoStageCompressed`→
    `.swiftMetalKernel`; `TurboQuantBackend` adds `.polarWHTReference`/`.metalPolarWHT`→QJL bucket;
    `KVLayerCodec` adds `.affineK8Vx`/`.affineK8VxResidual`→`.affineK8V4`).
  - **VALIDATED:** `xcodebuild ... build` (generic/iOS) **BUILD SUCCEEDED** at the new pins, AND a device
    build (`platform=iOS,id=GBU-12`) **signs + succeeds** (identity `maxwendler66@gmail.com`) — so the
    integration is proven and the device path is viable (signing works, unlike the prior wave0
    `failed_environmental`).
  - **REMAINING (the A-series cycle, deferred by user):** run the on-device bench (`run-ios-turboquant-bench.sh`,
    GBU-12) + the Mac TurboQuant tests against the new pins → assemble `compatibility-pair.json` from that
    REAL evidence (status stays `failed`/`unverified` — compressed throughput is not at FP16 parity) →
    `TurboQuantPinDriftTests` passes → commit the working-tree changes + push. Self-speculation ships
    default-off, so the working tree is behavior-inert until enabled + device-validated.
