...
- **Admission rule (data-backed by N1):** floor default 8192 (prefetch erases the ≥8K regression;
  ...
  tuning constant.
- **Pines hand-off (NOT applied — see why below). Ready-to-apply, but it is a cross-repo
  coordination step, not a safe autonomous edit.** Investigated 2026-06-07: pines is on
  `tq/real-device-evidence-acceptance` with **~901 lines of uncommitted prior-agent WIP**, and the
  mlx-swift-lm pin is an **evidence-coupled 5-site system** kept consistent by `TurboQuantPinDriftTests`:
  `project.yml`, `Pines.xcodeproj/project.pbxproj` (`revision = …;`), `…/swiftpm/Package.resolved`,
  the turboquant doc, AND a **generated** `compatibility-pair.json` (currently `status:"failed"`,
  `mlxSwiftLM.commit = 725add5`, `mlxSwift.commit = 609e833`). Applying the bridge edit without bumping
  the pin breaks the build (the field doesn't exist at `725add5`); hand-editing `compatibility-pair.json`
  would fabricate validation evidence (it is regenerated by the pines validation harness). So the correct
  sequence (user-coordinated, alongside reconciling the WIP):
  1. Bump mlx-swift-lm revision `725add5` → `ea8ecfc` (or later) in ALL of: `project.yml`,
     `project.pbxproj`, `Package.resolved`, and the turboquant doc; reconcile the separate **mlx-swift**
     pin (compat shows `609e833`) with what mlx-swift-lm@`ea8ecfc` resolves.
  2. In `MLXRuntimeBridge.swift` `GenerateParameters(...)` (BOTH the `#if PINES_TQ_WAVE6_API` and `#else`
     branches), add behind a profile/env flag:
     `selfSpeculationMode: (promptTokenCount >= 12288) ? .promptLookup : .off,`
     `selfSpeculationMinPromptTokens: 12288, selfSpeculationPrefetch: true` (greedy/no-processor ships
     exact; temp>0 + non-trimmable hybrid fall back automatically via `makeGenerationIterator`).
  3. Run the pines validation harness to regenerate `compatibility-pair.json` (real evidence, not hand-
     edited) → `TurboQuantPinDriftTests` passes; then the app xcodebuild + output-identity check.
