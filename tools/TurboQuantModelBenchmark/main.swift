...
if availability.supportsMetalPolarQJLAttention {
    do {
        for headDimension in TurboQuantRuntimeProbeResult.throughputOptimizedOnlineFusedHeadDimensions {
            ...
            let keys = MLXArray(
                values(count: 1 * 2 * 256 * headDimension, scale: 0.007),
                [1, 2, 256, headDimension]
            )
            let valuesArray = MLXArray(
                values(count: 1 * 2 * 256 * headDimension, scale: 0.011, phase: 0.2),
                [1, 2, 256, headDimension]
            )
            let decodeQueries = MLXArray(
                values(count: 1 * 4 * 1 * headDimension, scale: 0.019),
                [1, 4, 1, headDimension]
            )
            let prefillQueries = MLXArray(
                values(count: 1 * 4 * 64 * headDimension, scale: 0.017, phase: 0.3),
                [1, 4, 64, headDimension]
            )
            let scale = 1 / sqrt(Float(decodeQueries.dim(-1)))
            ...
            results.append(
                BenchmarkResult(
                    name: "cache.d\(headDimension).update_compressed",
                    status: "ok",
                    shape: prefill.shape,
            ...
            results.append(
                BenchmarkResult(
                    name: "cache.d\(headDimension).decode_attention",
                    status: "ok",
                    shape: output.shape,
                    latencySeconds: attentionLatency
                ))

            let (prefillAttentionLatency, prefillOutput) = try timed(iterations: iterations) {
                try turboQuantMetalScaledDotProductAttention(
            ...
            results.append(
                BenchmarkResult(
                    name: "cache.d\(headDimension).prefill_attention_q64",
                    status: "ok",
                    shape: prefillOutput.shape,
                    latencySeconds: prefillAttentionLatency
                ))
        }
